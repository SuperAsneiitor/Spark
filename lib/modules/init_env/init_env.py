...
与其他阶段的本质区别：
  - 其他阶段在 <project_root>/work/<case_name>/<case_version>/<stage_name>/ 下建立独立沙箱
...
class InitEnvComponent(BaseComponent):
    ...
    路径约定（覆盖基类）：
      ...
      root_dir     → project_root/work     (继承自基类)
      case_root    → work/<case>/<version> (子模块沙箱公共前缀，init_env 可预建)
    """
    ...
    def __init__(self, config: SparkConfig):
        ...
        # 基类会在 work/<case>/<version>/init_env 预建目录，此处清理
        _spurious = self.case_root / "init_env"
        if _spurious.exists():
    ...
    def run(self) -> None:
        ...
        logger.info(f"  Case 沙箱根 : {self.case_root}")
        logger.info(f"  work 顶层   : {self.root_dir}")
